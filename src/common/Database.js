...
class Database {
  ...
  addQuery(commandId, queryText) {
    this.db
      ...
      .getById(commandId)
      .get("queries")
      .insert(this.getQueryPrototype(queryText))
      .write();
    ...
    entities.forEach((ex) => {
      if ("entities" in ex) {
        var query = this.db
          ...
          .getById(commandId)
          .get("queries")
          .find({ text: ex.text }) // FIXME: Match using queryId, instead of text (will need to send queryId to backend)
          .get("entities");
  ...
  updateBackupQuery(commandId, parameter, backupQuery) {
    this.db
      ...
      .get("parameters")
      .find({ name: parameter })
      .assign({ backupQuery: backupQuery })
      .write();
    builder.build();
    return this.getCommandForId(commandId);
  }
  ...
  toggleContextAttribute(commandId, attribute) {
    ...
    this.updateContextType(
      commandId,
      attributes.length === 0 ? ContextType.Element : ContextType.Attribute
    );
  ...
  clearContextInfo(commandId) {
    ...
    this.updateCommandContext(commandId, {
      type:
        command.contextInfo.type === ContextType.Text
          ? ContextType.Text
          : ContextType.Element,
      selector: "*",
      allAttributes: [],
    ...
    this.sendBackendRequest(
      ...
      (xhr, error) => {
        if (error) {
          ...
          this.parseEntityResponse(
            commandId,
            json["rasa_nlu_data"]["common_examples"].filter(
              (ex) => ex.intent === command.name
            )
          );
        }
        callback(error);
