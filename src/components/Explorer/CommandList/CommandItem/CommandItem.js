...
export default class CommandItem extends Component {
  renderMarker() {
    if (this.props.command.type === "demo") {
      ...
      return (
        <Marker
          file={this.props.command.file}
          triggerFn={this.props.command.triggerFn}
          params={this.props.command.parameters.map((p) => p.name)}
          placement="top-end"
        />
      );
    }
  ...
  renderSummary() {
    if (this.props.command.type === "demo") {
      return (
        <span>
          <span className="fn">
            {createCountMessage(this.props.command.elements.length, "action")}
          </span>
          <span className="fn">
            {createCountMessage(
              this.props.command.parameters.length,
              "parameter"
            )}
          </span>
        </span>
  ...
  render() {
    return (
      <div className="cmdItem">
        ...
        <span className="cmdMarker">{this.renderMarker()}</span>
        <p className="cmdPath"> {path.basename(this.props.command.file)} </p>
        {this.renderSummary()}
        <div
          className="deleteBtn"
          onClick={() => this.props.delete(this.props.command.id)}
        >
