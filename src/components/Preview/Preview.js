...
export default class Preview extends Component {
  ...
  navigate() {
    ...
    if (this.state.address === "") {
      ...
      !this.state.address.startsWith("file://")
    ) {
      this.setState(
        (state, _props) => ({ address: `http://${state.address}` }),
        () => {
  ...
  openDevTools() {
    this.preview.isDevToolsOpened()
      ? this.preview.closeDevTools()
  ...
  createDemoCommand(event) {
    ...
    if (elements.length === 0) {
      ...
      this.setState({
        demoCommand: database.addDemoCommand(
          elements,
          parameters,
          this.state.address.split("/").pop()
        ),
  ...
  getRecordIcon() {
    switch (this.state.recordState) {
      case this.STOPPED:
        return (
          <FontAwesomeIcon icon={faMousePointer} size="lg"></FontAwesomeIcon>
        );
      case this.RECORDING:
  ...
  render() {
    var addressBar = !(this.state.recordState === this.RECORDING) ? (
      <input
        ...
        onChange={(event) => this.changeAddressBar(event.target.value)}
        onKeyPress={(event) => {
          if (event.key === "Enter") {
            this.navigate();
            event.target.blur();
      ...
      <p id="recordTutorial">
        {this.state.demoMessage != null
          ? this.state.demoMessage
          : "Click a UI element to start recording a command"}
      </p>
    );

    var content =
      this.state.demoCommand != null ? (
        <DemoPopover
          command={this.state.demoCommand}
          flipSide={true}
    ...
    var buttons =
      this.state.recordState === this.RECORDING ? (
        <div className="buttons">
          {addressBar}
          <div className="pill">{this.state.currentTag}</div>
          <div className="pill">
            {this.state.numClicks +
              (this.state.numClicks === 1 ? " click" : " clicks")}
          </div>
          <button
          ...
          <Tippy
            ...
            inertia={true}
            interactive={true}
            isVisible={this.state.recordState === this.POPOVER}
          >
            <button
              title="Create Demo Command"
              className="previewBtn"
              onClick={this.getRecordOnClick()}
            >
              {this.getRecordIcon()}
            </button>
          </Tippy>
        ...
        <div className="buttons">
          ...
          <button title="Go Back" className="previewBtn" onClick={this.goBack}>
          ...
          <button
            title="Reload"
            className="previewBtn"
            onClick={this.reloadPreview}
          >
            <FontAwesomeIcon icon={faRedoAlt} size="lg"></FontAwesomeIcon>
          </button>
          {addressBar}
          <button
            title="Toggle Developer Tools"
          ...
          >
            <button
              title="Record Demonstration Intent"
              className="previewBtn"
              onClick={this.getRecordOnClick()}
            >
              {this.getRecordIcon()}
