...
/* Remove highlights */
function clearMasks() {
  var hwrappersLength = document.getElementsByClassName("highlight-wrap")
    .length;
...
function extractContextInfo(element) {
  ...
  attributes.forEach(
    (attr) => (attributeExamples[attr] = element.getAttribute(attr))
  );
...
function shareContext() {
  ...
  Array.from(contextElement.children).forEach((el) =>
    contexts.push(extractContextInfo(el))
  );
