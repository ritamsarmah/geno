    ...
    if (
      ...
      if ("init" in declarator && declarator.init != null) {
        ...
        if (
          ...
          "callee" in declarator.init &&
          declarator.init.callee.name === "Function"
        ) {
          functions.push({
            name: declarator.id.name,
            params: declarator.init.arguments
              .slice(0, declarator.init.arguments.length - 1)
              .map((arg) => arg.value),
            lineNumber: declarator.loc.start.line,
          });
        }
        ...
        <div>
          <div className="filename centered">
            <span
              id="saveButton"
              style={{ color: "white" }}
              onClick={() => this.saveFile(this.state.file, true)}
            >
              <FontAwesomeIcon icon={faSave} size="lg" />
            </span>
            {path.basename(this.state.file)}
            <AddCommandButton
              relativePath={path.relative(this.props.dir, this.state.file)}
