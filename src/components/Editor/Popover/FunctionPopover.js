...
export default class FunctionPopover extends Popover {
  ...
  changeBackupQuery(event, paramName) {
    this.setState({
      command: database.updateBackupQuery(
        this.state.command.id,
        paramName,
        event.target.value
      ),
  ...
  toggleContextAttribute(attribute) {
    this.setState({
      command: database.toggleContextAttribute(
        this.state.command.id,
        attribute
      ),
    });
  ...
  trainModel(e) {
    ...
    database.trainModel(this.state.command.id, (error) => {
      this.setState({
        buttonValue: `Train Model (${error ? "Failed" : "Success"})`,
      });
      if (error) {
        window.alert(error);
      }
        ...
        <div className="popover">
          <form className="popoverForm">
            ...
            <p className="popoverSubtitle">
              Add follow up questions to ask when a parameter is not provided by
              user.
            </p>
            {this.state.command.parameters.map((p) => {
              return (
                <div key={p.name}>
                  ...
                  <input
                    type="text"
                    defaultValue={p.backupQuery}
                    placeholder={`e.g. What is ${p.name}?`}
                    onChange={(event) => this.changeBackupQuery(event, p.name)}
                  ></input>
                </div>
              );
            })}
