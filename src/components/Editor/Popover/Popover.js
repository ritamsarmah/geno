...
export default class Popover extends Component {
  ...
  toggleTrackingContext() {
    this.setState(
      (state, _props) => ({ isTrackingContext: !state.isTrackingContext }),
      () => {
  ...
  renderContextDropdown() {
    ...
    var selection =
      this.state.command.contextInfo.parameter != null
        ? this.state.command.contextInfo.parameter
        : "-";

    return (
      <select
        defaultValue={selection}
        onChange={this.onChangeContextParameter}
        style={{ width: "86%" }}
      >
        {names.map((name) => (
  ...
  renderAttributeSelect() {
    ...
    if (allAttributes.length === 0) {
      return <div className="attributeBox">No attributes available</div>;
    } else {
      return (
          ...
          {allAttributes.map((attr) => (
            <div key={`context-${attr}`}>
              <input
                type="checkbox"
                name={attr}
                defaultChecked={this.state.command.contextInfo.attributes.includes(
                  attr
                )}
  ...
  render() {
    ...
    var crossHairCursor =
      this.state.command.contextInfo.parameter != null ? "pointer" : "default";
        ...
        <div className="popover">
          <form className="popoverForm">
            ...
            <input
              id="commandNameInput"
              type="text"
              defaultValue={this.state.command.name}
              onChange={(e) =>
                this.changeCommandName(e.target.value.replace(/ /g, "_"))
              }
            ></input>
            ...
            <div>
              <div>
                <input
                  ...
                  onKeyUp={(e) => {
                    if (e.key === "Enter") this.onAddQuery();
                  }}
                ></input>
              ...
              <div className="listView">
                {this.state.command.queries.map((q) => (
                  <div
                    key={q.id}
                    className="listItem"
                    onClick={() => this.showAnalysis(q)}
                  >
                    {q.text}
                  </div>
                ))}
            ...
            <div>
              ...
              <div
                className="contextSelectText"
                style={{
                  display:
                    this.state.command.contextInfo.parameter == null ||
                    this.isContextDefault()
                      ? "none"
                      : "flex",
                }}
              >
                <Tippy
                  content={this.renderAttributeSelect()}
                  appendTo="parent"
                  interactive={true}
                  theme="light-border"
                  placement="right"
                ...
                >
                  <span
                    className={
                      "contextItem " +
                      (this.isContextDefault() ? "" : "customContext")
                    }
                    style={{
                      display: this.isContextDefault()
                        ? "none"
                        : "inline-block",
                      maxWidth: "85%",
                    }}
